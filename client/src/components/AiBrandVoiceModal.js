...
} from './brandVoiceModalHelpers';
import {
  clearPendingBrandVoiceToast,
  storePendingBrandVoiceToast,
} from '../toasts/brandVoicePendingToast';
...
const reloadWithPendingToast = (toasts, toast) => {
  const storedToast = storePendingBrandVoiceToast(toast);
  if (!storedToast) {
...
/**
 * Renders the Brand Voice modal, handles API calls, and applies chosen suggestions.
 */
export const AiBrandVoiceModal = ({
  fqcn,
  recordId,
  initialResult = null,
  initialContentHash = '',
  ...
  /**
   * Runs a draft Brand Voice check and replaces the modal result state with the response.
   */
  ...
  const brandVoiceConfigured = schemaConfig.state?.brandVoiceConfigured ?? true;
  ...
  const closeButton = (
    <button
      type="button"
      className="btn btn-close btn--icon-xl btn--no-text modal__close-button ai-brand-voice-modal__close"
      aria-label="Close"
      title="Close"
  ...
  return (
    ...
    >
      ...
      <ModalBody>
        {schemaError ? (
          <div className="ai-brand-voice-modal__banner ai-brand-voice-modal__banner--error">
            {schemaError}
          </div>
        ) : null}
        ...
        {!schemaError ? (
          <>
            {!brandVoiceConfigured ? (
              <div className="ai-brand-voice-modal__banner ai-brand-voice-modal__banner--warning">
                {schemaConfig.messages.missingBrandVoice}
              </div>
            ) : null}
            ...
            {isLoadingSchema || isChecking || isApplying ? (
              <div className="ai-brand-voice-modal__loading" role="status">
                <Spinner size="sm" />
            ...
            {!isLoadingSchema && !showResult ? (
              <p className="ai-brand-voice-modal__empty-state">{schemaConfig.messages.emptyState}</p>
            ) : null}

            {showResult ? (
              <div className="ai-brand-voice-modal__result">
                <p className="ai-brand-voice-modal__rating-value">{displayRating}</p>
                <p className="ai-brand-voice-modal__reasoning">{result.reasoningSummary}</p>

                {showAllAlignedMessage ? (
                  <div className="ai-brand-voice-modal__banner ai-brand-voice-modal__banner--success">
                    {schemaConfig.messages.allAligned}
                  </div>
                ) : (
                  <section className="ai-brand-voice-modal__section">
                    <h4 className="ai-brand-voice-modal__section-heading">{schemaConfig.labels.rewrite}</h4>
                    <p className="ai-brand-voice-modal__review-notice">{schemaConfig.messages.reviewNotice}</p>

                    {showNoSuggestionsMessage ? (
                      <p className="ai-brand-voice-modal__empty-state">{schemaConfig.messages.noSuggestions}</p>
                    ) : (
                      <div className="ai-brand-voice-modal__suggestions">
                        {actionableSuggestions.map((suggestion, index) => {
                          ...
                          const checkboxId = `ai-brand-voice-suggestion-${(suggestion.targetKey || `${index}`)
                            .replace(/[^a-zA-Z0-9_-]+/g, '-')}`;

                          return (
                            <article
                              key={suggestion.targetKey || `${suggestionHeading}-${index}`}
                              className="ai-brand-voice-modal__suggestion"
                            >
                              <div className="ai-brand-voice-modal__suggestion-header">
                                <div className="ai-brand-voice-modal__suggestion-heading">
                                  <h5>{suggestionHeading}</h5>
                                </div>

                                {supportsApply ? (
                                  <div className="ai-brand-voice-modal__suggestion-toggle">
                                    <input
                                      id={checkboxId}
                                      type="checkbox"
                                      checked={isSelected}
                              ...
                              <div
                                ...
                                className="ai-brand-voice-modal__suggestion-diff"
            ...
            {showApplyAction ? (
              <div className="ai-brand-voice-modal__footer-actions">
                <Button
                  color="info"
                  type="button"
                  onClick={handleApply}
...
export default connect(null, mapDispatchToProps)(AiBrandVoiceModal);
