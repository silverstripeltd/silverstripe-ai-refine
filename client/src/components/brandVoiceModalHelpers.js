...
export const defaultSchemaConfig = {
  ...
  messages: {
    ...
    checkFailure: 'Brand voice check failed',
...
/**
 * Reads the controller config section that seeds the Brand Voice modal.
 */
...
export const getModalConfig = () => ({
  ...
  modalClassName: 'ai-brand-voice-modal',
  size: 'xl',
  ...(getControllerConfig().form?.brandVoiceCheck || {}),
});
...
export const buildSchemaUrl = (fqcn, recordId, modalConfig = getModalConfig()) => (
  buildRecordActionUrl(fqcn, recordId, modalConfig.schemaUrl, '/admin/ai-brand-voice/schema')
);
...
export const buildCheckUrl = (fqcn, recordId, modalConfig = getModalConfig()) => (
  buildRecordActionUrl(fqcn, recordId, modalConfig.checkUrl, '/admin/ai-brand-voice/check')
);
...
export const buildApplyUrl = (fqcn, recordId, modalConfig = getModalConfig()) => (
  buildRecordActionUrl(fqcn, recordId, modalConfig.applyUrl, '/admin/ai-brand-voice/apply')
);
...
export const mergeSchemaConfig = (schemaPayload) => {
  const serverConfig = schemaPayload?.meta?.brandVoice || {};
  return {
    ...defaultSchemaConfig,
    ...serverConfig,
    messages: {
