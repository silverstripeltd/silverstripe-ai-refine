...
import { AiBrandVoiceModal } from '../../src/components/AiBrandVoiceModal';
import { PENDING_BRAND_VOICE_TOAST_KEY } from '../../src/toasts/brandVoicePendingToast';
...
test('disables check and apply while the edit form is dirty', async () => {
  ...
  render(<AiBrandVoiceModal {...createProps({
...
test('checks content, renders per-suggestion review cards, and applies selected suggestions', async () => {
  ...
  try {
    ...
    await waitFor(() => expect(global.fetch).toHaveBeenCalledTimes(1));
    await waitFor(() => expect(screen.getByRole('button', { name: 'Check Brand Voice' }).disabled).toBe(false));
    expect(screen.queryByRole('button', { name: 'Apply suggestions' })).toBeNull();

    fireEvent.click(screen.getByRole('button', { name: 'Check Brand Voice' }));
    ...
    expect(container.querySelectorAll('.ai-brand-voice-modal__suggestion-diff')).toHaveLength(1);
    expect(container.querySelector('.ai-brand-voice-modal__suggestion-diff del')?.textContent).toContain('Current paragraph');
    expect(container.querySelector('.ai-brand-voice-modal__suggestion-diff ins')?.textContent).toContain('Updated paragraph');
    expect(screen.queryByText('Brand voice rating')).toBeNull();
    expect(screen.queryByText('Reasoning summary')).toBeNull();
    expect(container.querySelector('textarea.ai-brand-voice-modal__reasoning')).toBeNull();
    expect(container.querySelector('.ai-brand-voice-modal__reasoning')?.tagName).toBe('P');
    ...
    expect(screen.getByRole('button', { name: 'Apply suggestions' }).getAttribute('color')).toBe('info');
    expect(container.querySelectorAll('.ai-brand-voice-modal__actions button')).toHaveLength(1);
    expect(container.querySelector('.ai-brand-voice-modal__footer-actions button')?.textContent).toBe('Apply suggestions');
    ...
    expect(props.actions.toasts.success).not.toHaveBeenCalledWith('Brand voice suggestions applied to draft content');
    expect(JSON.parse(window.sessionStorage.getItem(PENDING_BRAND_VOICE_TOAST_KEY))).toEqual({
      type: 'success',
...
test('renders a single diff row for each suggestion', async () => {
  ...
  const { container } = render(<AiBrandVoiceModal {...createProps({
  ...
  expect(container.querySelectorAll('.ai-brand-voice-modal__suggestion-diff ins')).toHaveLength(2);
  expect(screen.queryByText('Brand voice rating')).toBeNull();
  expect(screen.queryByText('Reasoning summary')).toBeNull();
  expect(container.querySelector('textarea.ai-brand-voice-modal__reasoning')).toBeNull();
  expect(container.querySelector('.ai-brand-voice-modal__reasoning')?.textContent).toBe('Mostly aligned.');
...
test('renders display labels for rating enums and keeps styled rewrite heading markup', async () => {
  ...
  expect(screen.getByText('Suggested rewrites').className).toContain('ai-brand-voice-modal__section-heading');
...
test('shows an aligned success banner when an Excellent result has no suggestions', async () => {
  ...
  await waitFor(() => expect(global.fetch).toHaveBeenCalledTimes(1));
  const banner = await screen.findByText('Your content fully aligns with the brand voice. No changes needed.');

  expect(banner.className).toContain('ai-brand-voice-modal__banner');
  expect(banner.className).toContain('ai-brand-voice-modal__banner--success');
...
test('shows the generic no-suggestions message for non-Excellent empty results', async () => {
  ...
  expect(screen.queryByText('Your content fully aligns with the brand voice. No changes needed.')).toBeNull();
});
...
test('shows schema load failures in the modal banner and toast state', async () => {
  ...
  expect(screen.queryByRole('button', { name: 'Check Brand Voice' })).toBeNull();
});
...
test('clears stale cached results when the saved draft hash has changed', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Check Brand Voice' })).not.toBeNull();
});
...
test('shows a warning toast for partial apply responses that do not reload the cms', async () => {
  ...
  try {
    ...
    await waitFor(() => expect(global.fetch).toHaveBeenCalledTimes(2));
    await waitFor(() => expect(screen.getByRole('button', { name: 'Apply suggestions' }).disabled).toBe(false));
