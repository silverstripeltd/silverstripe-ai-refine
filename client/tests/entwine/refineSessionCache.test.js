...
import {
  ...
  hasRecordContext,
} from '../../src/entwine/refineSessionCache';

test('createRefineSessionCache stores and returns the latest result', () => {
  const cache = createRefineSessionCache();
  const result = {
    rating: 'Good',
...
test('createRefineSessionCache clears cached results when invalidated', () => {
  const cache = createRefineSessionCache({
    result: {
      rating: 'Good',
      reasoningSummary: 'Mostly aligned.',
