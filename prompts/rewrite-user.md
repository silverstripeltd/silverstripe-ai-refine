Assess the website content against the supplied refine definition.

Choose exactly one rating from this scale:
- Excellent - you cannot find any sentences or phrases that break the refine. The tone, word choices, and structure all match, so zero rewrites are needed and every target should be returned unchanged.
...
- "reasoningSummary": 2-4 sentences explaining the rating. Mention specific strengths and weaknesses and refer to the refine definition where relevant.
...
- If a target already matches the refine, return the original sourceContent unchanged as the suggestedContent
...
- Only rewrite content that does not match the refine. If the source content already matches, return it unchanged.

=== REFINE_DEFINITION_START ===
{refineDefinition}
=== REFINE_DEFINITION_END ===
