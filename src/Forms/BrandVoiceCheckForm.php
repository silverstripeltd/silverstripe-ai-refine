...
namespace SilverstripeLtd\AiBrandVoice\Forms;
...
/**
 * Builds the server-side schema for the on-demand Brand Voice modal.
 */
class BrandVoiceCheckForm extends Form
{
    ...
    public const CHECK_BUTTON_LABEL = 'Check Brand Voice';
    public const RECHECK_BUTTON_LABEL = 'Re-check Brand Voice';
    public const APPLY_BUTTON_LABEL = 'Apply suggestions';
    ...
    public const APPLY_SUCCESS_MESSAGE = 'Brand voice suggestions applied to draft content';
    public const APPLY_PARTIAL_MESSAGE = 'Some suggestions could not be applied';
    public const APPLY_FAILURE_MESSAGE = 'Unable to apply brand voice suggestions';
    ...
    public const RATING_LABEL = 'Brand voice rating';
    ...
    public static function createForRecord(
        BrandVoiceController $controller,
        DataObject $record,
        bool $brandVoiceConfigured
    ): self {
        $fields = FieldList::create(
            LiteralField::create(
                'BrandVoiceDraftNotice',
                self::renderBanner(self::DRAFT_NOTICE, 'info')
            ),
            LiteralField::create(
                'BrandVoiceEmptyState',
                HTML::createTag(
                    'p',
                    ['class' => 'ai-brand-voice-modal__empty-state'],
                    Convert::raw2xml(self::EMPTY_STATE_MESSAGE)
                )
            ),
            ReadonlyField::create('RatingDisplay', self::RATING_LABEL, '')
                ->addExtraClass('ai-brand-voice-modal__rating-field'),
            TextareaField::create('ReasoningSummaryDisplay', self::REASONING_LABEL)
                ->setRows(self::REASONING_ROWS)
                ->setAttribute('readonly', 'readonly')
                ->addExtraClass('ai-brand-voice-modal__reasoning-field'),
            TextareaField::create('RewrittenContentDisplay', self::REWRITE_LABEL)
                ->setRows(self::REWRITE_ROWS)
                ->setAttribute('readonly', 'readonly')
                ->addExtraClass('ai-brand-voice-modal__rewrite-field'),
            LiteralField::create(
                'BrandVoiceCopyAffordance',
                HTML::createTag(
                    'p',
                    ['class' => 'ai-brand-voice-modal__copy-affordance'],
                    Convert::raw2xml(self::COPY_BUTTON_LABEL)
                )
            ),
            HiddenField::create('BrandVoiceConfigured', '', $brandVoiceConfigured ? '1' : '0')
        );

        $actions = FieldList::create(
            FormAction::create('BrandVoiceCheckAction', self::CHECK_BUTTON_LABEL)
        ...
        $form->addExtraClass('form--no-dividers ai-brand-voice-modal__schema');
        $form->loadDataFrom([
            'RatingDisplay' => '',
            'ReasoningSummaryDisplay' => '',
            'RewrittenContentDisplay' => '',
            'BrandVoiceConfigured' => $brandVoiceConfigured ? '1' : '0',
        ]);
        return $form;
    }
    ...
    private static function renderBanner(string $message, string $variant): string
    {
        return HTML::createTag(
            'div',
            ['class' => sprintf('ai-brand-voice-modal__banner ai-brand-voice-modal__banner--%s', $variant)],
