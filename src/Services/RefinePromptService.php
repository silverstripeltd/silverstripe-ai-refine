...
class RefinePromptService
{
    ...
    private function renderPromptTemplate(
        ...
        return trim(str_replace(
            ['{refineDefinition}', '{pageTitle}', '{content}', '{rewriteTargets}'],
            [
                trim($refineDefinition),
                trim($pageTitle),
                trim($content),
    ...
    private function serialiseRewriteTargets(array $rewriteTargets): string
    {
        return (string) json_encode(
            array_map(
                static fn(RefineRewriteTarget $target): array => $target->toPromptPayload(),
                $rewriteTargets
            ),
