...
namespace SilverstripeLtd\AiRefine\Tests\Reports;

use SilverstripeLtd\AiRefine\Models\RefineAnalysis;
use SilverstripeLtd\AiRefine\Reports\RefineReport;
...
/**
 * Covers the refine CMS report.
 */
class RefineReportTest extends SapphireTest
{
    ...
    /**
     * Logs in an admin and seeds a valid refine definition.
     */
    protected function setUp(): void
    {
        ...
        $siteConfig->RefineDefinition = 'We write with a clear, practical, and steady voice that keeps '
            . 'content concise while still sounding helpful and human.';
        $siteConfig->write();
    }

    /**
     * Clears the configured refine after each report test.
     */
    ...
    public function testReportSortsWorstFirstAndFiltersRatings(): void
    {
        ...
        $this->assertNotSame($poor->getRefineAnalysis()->ReasoningSummary, $rows->toArray()[0]->Reasoning);
    ...
    /**
     * Confirms the report shows an empty-state banner when no refine is configured.
     */
    public function testReportShowsBannerWhenNoRefineConfigured(): void
    {
        $siteConfig = SiteConfig::current_site_config();
        $siteConfig->RefineDefinition = '';
        $siteConfig->write();
        ...
        $banner = $fields->fieldByName('RefineBanner');
    ...
    {
        $hidden = RestrictedRefinePage::create(['Title' => 'AAA hidden needs work']);
        $hidden->write();
        $hiddenAnalysis = $hidden->getOrCreateRefineAnalysis();
    ...
    /**
     * Creates a page fixture with persisted refine analysis data.
     */
