 ...
 */
class BrandVoiceEvaluationServiceTest extends SapphireTest
{
    ...
    {
        $provider = new StubProvider(new BrandVoiceFullResult('Excellent', 'Should not be used.', []));
        $service = new BrandVoiceEvaluationService(new ContentExtractionService(), new StubProviderFactory($provider));
    ...
    {
        $provider = new StubProvider(
            new BrandVoiceFullResult('Excellent', 'Strong alignment.', [])
        );
        $service = new BrandVoiceEvaluationService(new ContentExtractionService(), new StubProviderFactory($provider));
    ...
    public function testEvaluateDraftReturnsResolvedSuggestions(): void
    {
        $provider = new StubProvider(new BrandVoiceFullResult('Good', 'Mostly aligned.', [
            new BrandVoiceSuggestion('page:title', 'page_title', '', null, '', 'Updated draft title'),
            new BrandVoiceSuggestion('page:content', 'page_content', '', null, '', '<p>Updated draft body</p>'),
        ]));
        $service = new BrandVoiceEvaluationService(new ContentExtractionService(), new StubProviderFactory($provider));
    ...
    public static function provideInvalidDraftSuggestions(): array
    {
        return [
            'unexpected target key' => [[
                new BrandVoiceSuggestion('page:title', 'page_title', '', null, '', 'Updated draft title'),
                new BrandVoiceSuggestion('page:unknown', 'page_content', '', null, '', 'Updated draft body'),
            ], 'AI provider response referenced unexpected target page:unknown'],
            'wrong target type' => [[
                new BrandVoiceSuggestion('page:title', 'page_content', '', null, '', 'Updated draft title'),
                new BrandVoiceSuggestion('page:content', 'page_content', '', null, '', '<p>Updated draft body</p>'),
            ], 'AI provider response returned the wrong targetType for page:title'],
            'duplicate suggestion' => [[
                new BrandVoiceSuggestion('page:title', 'page_title', '', null, '', 'Updated draft title'),
                new BrandVoiceSuggestion('page:title', 'page_title', '', null, '', 'Duplicate title'),
    ...
    public function testEvaluateDraftRejectsInvalidProviderSuggestions(
        ...
        $provider = new StubProvider(new BrandVoiceFullResult('Good', 'Mostly aligned.', $suggestions));
        $service = new BrandVoiceEvaluationService(new ContentExtractionService(), new StubProviderFactory($provider));
